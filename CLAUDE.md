...
- **Agent rubric (Anthropic `agent-development`)**: every agent body should contain a `## When to invoke` section listing 2–4 worked scenarios as prose bullets and a `Do NOT use this agent for:` line. Body length **must** stay under 10 000 chars; supervisors with phase-by-phase content should extract per-phase narrative into `claude-catalog/docs/<topic>/`. Body length is enforced via a **ratchet rule** in `validate_catalog.py`: agents listed in [`.github/scripts/legacy-body-baseline.json`](.github/scripts/legacy-body-baseline.json) may exceed 10 000 chars up to their per-file baseline (warning only); any growth past the baseline is a **hard error**, and any agent not on the baseline that exceeds 10 000 chars is a **hard error**. When extracting a legacy file below 10 000, REMOVE its entry from the baseline so a future regrowth is caught immediately. For the extraction recipe, see [`claude-catalog/docs/supervisor-extraction-template.md`](claude-catalog/docs/supervisor-extraction-template.md).
