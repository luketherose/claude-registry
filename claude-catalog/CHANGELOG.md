...
- **Refactoring workflow supervisor (1 new agent, beta)** — top-level workflow agent for end-to-end refactoring/migration on a codebase. One layer above the phase supervisors: delegates each phase sequentially via the Agent tool, never performs analysis itself. Strict human-in-the-loop with mandatory pre-phase brief (including a schematic of the upcoming phase's parallelization) and post-phase recap (with next-phase preview). Designed for extension to later phases:
  - `refactoring-supervisor@0.3.0` — opus, single entrypoint for `refactoring-supervisor`-driven workflows; coordinates Phase 0 (`indexing-supervisor`), Phase 1 (`functional-analysis-supervisor`), Phase 2 (`technical-analysis-supervisor`), and Phase 3 (`baseline-testing-supervisor`); maintains a workflow manifest at `docs/refactoring/workflow-manifest.json`; refuses unimplemented phases rather than inventing. Bumped from 0.2.0 to register Phase 3 in the phase registry, add the Phase 3 schematic, update bootstrap detection, and add a **mandatory execution-timing block** in the post-phase recap (per-step durations from the phase manifest, plus cumulative workflow time across phases). The recap surfaces the finest granularity each phase manifest exposes (Phase 3 manifests include per-agent timing; older phase manifests may only expose wave-level timing).
