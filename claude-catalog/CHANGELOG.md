...
- **Refactoring workflow supervisor (1 new agent, beta)** — top-level workflow agent for end-to-end refactoring/migration on a codebase. One layer above the phase supervisors: delegates each phase sequentially via the Agent tool, never performs analysis itself. Strict human-in-the-loop with mandatory pre-phase brief (including a schematic of the upcoming phase's parallelization) and post-phase recap (with next-phase preview). Designed for extension to later phases:
  - `refactoring-supervisor@0.2.0` — opus, single entrypoint for `refactoring-supervisor`-driven workflows; coordinates Phase 0 (`indexing-supervisor`), Phase 1 (`functional-analysis-supervisor`), and Phase 2 (`technical-analysis-supervisor`); maintains a workflow manifest at `docs/refactoring/workflow-manifest.json`; refuses unimplemented phases rather than inventing. Bumped from 0.1.0 to register Phase 2 in the phase registry, add the Phase 2 schematic, and update bootstrap detection.
