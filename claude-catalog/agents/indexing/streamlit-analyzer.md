...
description: >
  Use to analyze Streamlit-specific
  concerns: pages, session_state usage, widgets, caching, navigation, custom
  components, and migration-relevant anti-patterns. Critical for migration
  since Streamlit's reactive script-as-page model has no Angular equivalent.
  Invoked only when Streamlit is detected in Phase 0.
