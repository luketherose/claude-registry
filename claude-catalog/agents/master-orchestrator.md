...
- If the user says nothing about formats → **always default to `["md", "tex", "docx", "html"]`**
...
> **HARD RULE**: Never pass `output_formats=["md"]` or any single-format restriction
> to `functional-analyst` unless the user has **explicitly** written "only md" or "markdown only"
> in their request. The `.docx` (Word) deliverable is always expected unless the user opts out.
> When in doubt, produce all available formats.
...
- Pass a single-format restriction (`output_formats=["md"]`) to `functional-analyst` unless
  the user has explicitly opted out of other formats. The default is always all available formats.
