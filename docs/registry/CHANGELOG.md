...
`claude-catalog/` and `claude-marketplace/` are collapsed into `plugins/`, with
`.claude-plugin/marketplace.json` at the root and a `plugin.json` per plugin. Install with
`/plugin marketplace add` and `/plugin install`; updates arrive in the background. The
`setup-capabilities.sh` installer and the `SessionStart` git-diff sync hook are retired.

**Skills.** All 42 skills are converted from `haiku` subagents to Agent Skills at
`plugins/<plugin>/skills/<name>/SKILL.md`. `model`, `tools` and `color` are removed from
skill frontmatter. Six oversized skills are split into `references/`, bringing every
`SKILL.md` body under the 500-line guidance. Agents now load skills with the `Skill` tool;
four agents that need a skill on every run preload it via the `skills:` frontmatter field.

**Bundled references.** 123 references to `claude-catalog/docs/**` across 62 files are
rewritten to `${CLAUDE_PLUGIN_ROOT}/references/**`. Those paths previously did not resolve
at runtime, because the installer copied only agent files.

**Description budget.** Combined subagent descriptions drop from ~14200 tokens to ~10200,
against a 15000-token platform ceiling. Per-plugin enabling means a typical development
session now loads ~1900 tokens of descriptions instead of ~14200.

**Frontmatter.** `model: inherit` for user-facing agents, `opus` plus `effort: high` for
supervisors, challengers, auditors and deliberation personas, `sonnet` retained for
fan-out workers. Supervisors gain `experimental.cacheTtl: 1h`. The non-standard
`model_justification` field is removed; model rationale moves into an HTML comment.

**Deliberation.** The seven deliberation capabilities are moved to `opus`, matching what
`deliberative-decision-engine` already documented ("Default model tier is Opus for every
persona") but did not configure.

### Added

- `plugins/replatforming/skills/python-to-{java,angular,react}-migration-expert`, merged
  and converted from the retired `skill-registry` repository.
- `.github/scripts/validate_registry.py`, replacing `validate_catalog.py` and
  `validate_marketplace.py`. Gates the description budget, `SKILL.md` length, frontmatter
  correctness, reference-link resolution and `${CLAUDE_PLUGIN_ROOT}` path resolution.

### Removed

- `code-reviewer`. Superseded by `pr-review-toolkit@claude-plugins-official`.
- Per-capability `beta` and `stable` tiers. Versioning is semver on the plugin.
- `claude-marketplace/` in full, and the one-off bulk-edit scripts (moved to `archive/`).

## [Unreleased, earlier]
### Added
- **`refactoring-workflow/retrospective.md`** (new) — Workflow Retrospective protocol for `refactoring-supervisor`. Runs automatically after Phase 4 Step 6 PO sign-off (Step G). Synthesizes findings across all five phases from existing artifacts (no sub-agents dispatched), classifies issues by severity (`blocking / high / medium / low`), writes `docs/refactoring/retrospective.md`, and presents three choices: `close`, `iterate` (re-run from earliest affected phase), `defer-and-close`. Iteration loop has no cap; each cross-phase iteration produces a new retrospective.
- **`refactoring-workflow/cross-phase-iteration.md`** (new) — Cross-phase iteration protocol. When the retrospective routes to `iterate`, this doc drives re-running phases N through 4 in sequence, carrying the retrospective delta to each phase supervisor. Covers: entry-point calculation (earliest affected phase), archive policy (existing outputs renamed to `-archive-iter<N>/` before overwrite), Phase 4 re-run granularity (full from Step 0 if Phase 1/2 changed; Step 6 only if Phase 3 changed), cross-phase delta schema (`docs/refactoring/_meta/cross-phase-delta-iter<N>.json`), state management in `workflow-manifest.json`, and deliberation routing for contested adjustments.
### Changed
- **`refactoring-supervisor@3.3.0`** — two new reference docs added to the reference table (`retrospective.md`, `cross-phase-iteration.md`); description updated to mention Workflow Retrospective and cross-phase iteration.
- **`refactoring-workflow/per-phase-protocol.md`** — Step F (Phase 4): no longer ends the workflow on PO sign-off; routes to new Step G. **New Step G — Workflow Retrospective** added: auto-entry after Phase 4 sign-off, three HITL choices, loop-back after cross-phase iteration.
- **`refactoring-workflow/decision-rules.md`** — new section `## Retrospective and cross-phase iteration` with 11 decision rows covering: retrospective sign-off paths, close/iterate/defer-and-close options, blocking-issue override rule, Phase 4 re-run scope mapping, Phase 0 escalation, and cross-phase iteration cap recommendation.
- **`refactoring-workflow/supervisor-protocol.md`** — workflow phases table updated (Row G: Workflow Retrospective); added explanatory paragraphs for Workflow Retrospective, cross-phase iteration, and updated the "No Phase 5" + "Unimplemented phases" notes.
- **`bmad/workflows.json`** — new `retrospective` quality-gate entry in the `phases` array with `triggers`, `can_trigger_cross_phase_iteration`, and `cross_phase_iteration_reentry` fields.

### Added (old entry below)
- **`refactoring-workflow/supervisor-protocol.md`** — new reference doc for `refactoring-supervisor` containing: `## Pipeline state` (documents the `workflow-manifest.json` state mechanism, bootstrap protocol, and relationship with per-phase `pipeline-state.yaml`), `## Workflow phases` table (Phases 0–4 with supervisor, output root, status), `## Escalation triggers`, and `## Output format`. Completes the BMAD progressive-disclosure extraction for all 7 supervisor-protocol.md files.
- **`claude-catalog/evals/functional-analysis-supervisor/evals.json`** — added eval-002 (exports-only resume: detects complete analysis, offers export-only mode without re-running sub-agents) and eval-003 (missing `.indexing-kb/` guard: stops and asks user, does not auto-invoke indexing pipeline).
- **`claude-catalog/templates/new-use-case/evals/evals.json`** — eval stub template for new use cases (2 scenarios: primary full-pipeline + resume/partial-run).

### Changed
- **`README.md`** — updated repository structure block to include `bmad/`, `evals/`, `templates/new-use-case/`; agent count 83 → 87, skills count updated; added BMAD methodology layer section; fixed descriptions for `refactoring-supervisor` (Phases 0–5 → 0–4), `baseline-testing-supervisor` (Phase 5 → Phase 4 Step 6), `refactoring-tobe-supervisor` and `tobe-testing-supervisor` marked LEGACY (v2 only).
- **12 review findings fixed** — DAG accuracy (`baseline-runner.preceded_by` adds `service-collection-builder`; `functional-traceability-auditor.preceded_by` adds W2 workers as fallback predecessors when challenger is skipped), Phase 5 terminology cleaned from `baseline-testing-supervisor.md` and its evals, `refactoring-supervisor.md` body cleaned to Role + When to invoke + Reference docs only, `quick-start.md` Phase 5 refs fixed, LEGACY callouts added to legacy supervisor-protocol.md files, `functional-analysis-supervisor/triggers.json` negative disambiguation added.
- **`guida-operativa.pdf` and `pitch-claude-registry.pptx` regenerated** — updated to reflect 87 agents / 42 skills, BMAD methodology layer, Phases 0–4 workflow, LEGACY status of v2 supervisors, new-use-case scaffold.

### Added
- **BMAD methodology layer (`bmad/`).** New top-level directory implementing BMAD progressive disclosure, document-as-cache, workflow DAG, and scalability patterns while remaining fully Anthropic-validator-compliant. Contains: `bmad/workflows.json` (machine-readable workflow registry), `bmad/design/mapping.md` (canonical BMAD→Anthropic pattern decisions), `bmad/design/workflow-dag-draft.json` (dependency graph for all 87 pipeline agents), `bmad/scripts/backfill-dag.py` (DAG→catalog.json sync script).
- **`bmad` block in `catalog.json`.** 85 pipeline agents now carry a `bmad: {workflow, role, phase, wave, preceded_by[], followed_by[]}` block, making the full dependency graph machine-readable. Standalone agents carry an empty bmad block. Populated by `backfill-dag.py`.
- **`supervisor-protocol.md` reference docs** for all 6 phase supervisors and 2 legacy supervisors: `claude-catalog/docs/indexing/supervisor-protocol.md`, `docs/functional-analysis/supervisor-protocol.md`, `docs/technical-analysis/supervisor-protocol.md`, `docs/baseline-testing/supervisor-protocol.md`, `docs/deliberation/supervisor-protocol.md`, `docs/refactoring-tobe/supervisor-protocol.md`, `docs/tobe-testing/supervisor-protocol.md`. Each contains the full Decision rules, Escalation triggers, and Constraints sections extracted from the corresponding supervisor body.
- **Output-spec reference docs** for worker agents: `docs/indexing/module-documenter/output-spec.md`, `docs/indexing/synthesizer/output-spec.md`, `docs/functional-analysis/actor-feature-mapper/output-spec.md`, `docs/functional-analysis/user-flow-analyst/jsonl-output-spec.md`. These hold verbose output-format templates extracted from worker bodies.
- **New-use-case template** at `claude-catalog/templates/new-use-case/`: README (8-step guide), supervisor-template.md, worker-template.md, docs/supervisor-protocol.md, docs/dispatch-prompt-template.md, evals/triggers.json, catalog-entry-template.json. Enables adding a new workflow use case without modifying existing agents.
...
- **Phase D quality pass — 32 agent files improved.** Developer cluster (9 agents): description trigger phrases replaced with realistic user utterances; When to invoke bullets rewritten as concrete trigger-action pairs; Do NOT use lines now name specific sibling agents. Quality cluster (5 agents): same improvements; `technical-analyst.md` When to invoke fully rewritten (previous version described indexing-supervisor work, not standalone analysis). Documentation cluster (4 agents): description triggers fixed (malformed in document-creator/presentation-creator); Do NOT use lines now name specific alternatives. Worker agents (14 baseline-testing + tobe-testing workers): Do NOT use lines now explicitly state "not for standalone use — invoked only as part of [supervisor-name] (Wave N)".
- **Phase E2 — artifact evals completed.** `technical-analysis-supervisor/evals.json` (3 scenarios: full pipeline, exports-only resume, missing KB stop), `baseline-testing-supervisor/evals.json` (3 scenarios: full pipeline, existing baseline gate, critical failure escalation), `deliberative-decision-engine/evals.json` (3 scenarios: English high-risk debate, Italian trigger, non-trigger guard).

### Changed (BMAD migration — version bumps)
...
- **BMAD migration — 18 agents body-reduced (patch bumps):** `indexing-supervisor` 2.0.1→2.0.2, `module-documenter` 2.0.0→2.0.1, `synthesizer` 2.0.0→2.0.1, `functional-analysis-supervisor` 3.0.1→3.0.2, `actor-feature-mapper` 2.0.0→2.0.1, `user-flow-analyst` 2.0.0→2.0.1, `technical-analysis-supervisor` 2.1.1→2.1.2, `code-quality-analyst` 2.0.0→2.0.1, `dependency-security-analyst` 2.0.0→2.0.1, `integration-analyst` 2.0.0→2.0.1, `resilience-analyst` 2.0.0→2.0.1, `state-runtime-analyst` 2.0.0→2.0.1, `baseline-testing-supervisor` 0.3.1→0.3.2, `deliberative-decision-engine` 0.1.1→0.1.2, `refactoring-tobe-supervisor` 0.3.1→0.3.2, `tobe-testing-supervisor` 0.1.1→0.1.2, `decomposition-architect` 0.2.0→0.2.1, `logic-translator` 0.2.0→0.2.1.
...
### Added
- **Iterative HITL loop for analysis phases (Phases 1–3).** New canonical specs at `claude-catalog/docs/refactoring-workflow/iteration-loop.md` and `phase-verification-report.md`. Each analysis phase now ends with a structured `approve / iterate / stop` prompt instead of one-shot proceed/revise. The user may run unbounded iterations with explicit adjustments before approving. The phase verification report (`_meta/phase-verification-report.md`) is the canonical HITL document the human reads to decide.
- **Phase 1 feature narrative (`00b-feature-narrative.md`).** New human-readable entry-point document written by `functional-analysis-supervisor` in Wave 3c. Plain-prose, feature-by-feature chapters explaining what the application does, who uses it, what triggers it, what happens. The narrative is intentionally complementary to the structured use-case catalogue: precise IDs live in `02-features.md` / `06-use-cases/`; the human story lives in `00b-feature-narrative.md`. Inserted at slot `00b-` (between `00-context.md` and `01-actors.md`) so existing numbering is preserved and downstream consumers (baseline-testing, refactoring-tobe) keep working.
- **Deliberation extended to Phases 1–3.** `claude-catalog/docs/deliberation/integration-replatforming.md` now includes a § "Decision points (Phases 1–3)" covering contested actor/feature/UC definitions, contested risk severities, contested test dispositions, and scope-of-iteration disputes. Triggered automatically when the user's iteration adjustments contain a debate trigger or when an adjustment conflicts with a prior sub-agent output and resolution is subjective.
...
### Changed
- `refactoring-supervisor` (Phase 1–3 iteration support): decision rules table extended to cover `approve / iterate / stop`, `Resume mode: iterate`, deliberation routing for contested adjustments, and iteration counter tracking. The supervisor now drives the iteration loop documented in `iteration-loop.md` between every analysis phase's Wave 3d (Phase 1) / Wave 3c (Phase 2) / Wave 3b (Phase 3) and the next phase.
- `functional-analysis-supervisor` (v3.0.0 — iterative analysis): adds Wave 3c (feature narrative — supervisor writes `00b-feature-narrative.md` directly) and Wave 3d (verification report — supervisor writes `_meta/phase-verification-report.md`). Exports are now gated on `approve` (no longer regenerated on every iteration). New decision rules for `Resume mode: iterate`, snapshot-before-overwrite, and deliberation routing on contested adjustments.
- `technical-analysis-supervisor` (v2.1.0 — iterative analysis): adds Wave 3c (verification report). Exports gated on `approve`. New decision rules for `Resume mode: iterate` and deliberation routing on contested severities / cross-domain assignments.
- `baseline-testing-supervisor` (v2.0.0 — iterative analysis): adds Wave 3b (verification report) and Wave 4 (iteration handling). Oracle is now an explicit guarded artifact — iteration deltas touching oracle scope require user confirmation. No exports to gate (Phase 3 has no PDF/PPTX) but iteration loop applies the same `approve / iterate / stop` HITL.
- `docs/refactoring-workflow/per-phase-protocol.md`: Step E recap updated to surface iteration number and verification-report path. New Step E.5 (mandatory verification report for Phases 1–3) and rewritten Step F (the canonical iteration loop with `approve / iterate / stop`, structured delta capture, snapshot-before-overwrite, and optional deliberation routing). Phase 0 retains its one-shot confirmation; Phase 4 retains its per-step Step 3 sub-loop.
- `docs/functional-analysis/output-layout.md`: added `00b-feature-narrative.md` to the layout tree with full canonical structure spec and hard rules. Added `_meta/iteration-log.jsonl`, `_meta/phase-verification-report.md`, `_meta/snapshots/` to the `_meta/` layout. Numbering of existing artifacts is unchanged.
- `docs/functional-analysis/phase-plan.md`: added Wave 3c (narrative), Wave 3d (verification report), and Wave 4 (iteration handling). Re-dispatch mapping (which sub-agents re-run for which adjustment target) documented. Export Wave moved from "always ON at end" to "gated on approve".
- `docs/technical-analysis/phase-plan.md`: added Wave 3c (verification report) and Wave 4 (iteration handling). Export Wave gated on `approve`. Phase-2 customization of the canonical verification-report structure documented.
- `docs/baseline-testing/phase-plan.md`: added Wave 3b (verification report) and Wave 4 (iteration handling). Oracle-snapshot regeneration explicitly requires user confirmation in the iteration delta.
- `docs/deliberation/integration-replatforming.md`: new § "Decision points (Phases 1–3)" with the per-phase decision points routed to `deliberative-decision-engine` from inside the iteration loop.
...
- `streamlit-analyzer` updated: primary outputs `silver/framework-findings.jsonl` + `bronze/ui-surfaces.json` with `evidence_ids`; grounding instruction added (emit ui_surface evidence records, track session_state reads/writes across pages)
- `indexing-supervisor` updated for evidence-first Phase 0: grounding policy reference, large-file-policy reference, evidence-ledger constraint, indexing-auditor (Wave 4a) added, coverage gate, updated HITL format
- `codebase-mapper@2.0.0` (beta): primary outputs now in `bronze/` — adds `bronze/file-inventory.jsonl`, `bronze/large-files.jsonl`, `bronze/large-file-chunks.jsonl`, `bronze/parse-errors.jsonl`, `bronze/file-hashes.json`, `bronze/symbol-index.jsonl`, `bronze/test-inventory.jsonl`; evidence record emission to `evidence-ledger.jsonl` (kind: source_file, source_symbol, source_chunk); large-file policy integration (>800 lines / >150 KB threshold); `02-structure/` retained as backward-compat copy only on existing KBs. `output-templates.md` updated with bronze/ JSONL record templates for file-inventory, large-files, and large-file-chunks.
- `dependency-analyzer@2.0.0` (beta): primary outputs in `bronze/` (`bronze/import-graph.json`, `bronze/dependency-locks.json`); evidence record emission for each external dependency (kind: dependency); `03-dependencies/` retained as backward-compat copy only on existing KBs.
- `synthesizer@2.0.0` (beta): primary outputs in `gold/` (`gold/system-overview.md`, `gold/bounded-context-hypothesis.md`, `gold/complexity-hotspots.md`, `gold/unresolved-gaps.md`); Gold synthesis rule enforced — no new claims beyond bronze/silver content; `gold/unresolved-gaps.md` aggregates all gaps from `silver/gaps.jsonl`; `08-synthesis/` retained as backward-compat copy only on existing KBs.
...
- **`debate-replatforming-specialist@0.1.0` (beta)** — Migration / Replatforming Specialist persona (opus); evaluates 14 migration criteria including sequencing, cutover, rollback, data migration, integration risk, reversibility, modernisation vs lift-and-shift.
- **`debate-risk-reviewer@0.1.0` (beta)** — Security / Compliance / Risk Reviewer persona (opus); GDPR/HIPAA/PCI/SOX/contractual review with explicit `riskFlags.requiresJudgeArbitration` / `requiresHumanArbitration` authority.
- **`debate-operations-reviewer@0.1.0` (beta)** — Operations / Reliability Reviewer persona (opus); SLO, capacity, observability, RTO/RPO, partial-failure modes, cost-of-operation.
- **`debate-judge@0.1.0` (beta)** — Neutral judge persona (opus) with two modes: SUMMARISER (Step 3, must NOT recommend) and ARBITRATOR (Step 6, must explicitly address every unresolved high/critical objection in `unresolvedObjectionsAddressed` — engine rejects judge outputs that drop one).
- **`claude-catalog/docs/deliberation/`** — 7 reference docs read on demand by the engine: `trigger-lexicon.md` (IT/EN trigger lexicon, scoring rules, false-positive guards), `schemas.md` (authoritative JSON schemas for every artefact), `strategy-selection.md` (final-decision strategy selection rules + commit-protocol selection), `commit-protocol.md` (`local_transactional` default + `raft` / `pbft` extension-point contracts), `dispatch-templates.md` (per-persona dispatch prompt boilerplate for Steps 2/3/4/5/6), `output-layout.md` (audit-trail tree + redaction conventions), `integration-replatforming.md` (Phase-4 decision points routed to deliberation; programmatic invocation contract).
...
- **9 medium agents (13–16 k chars) trimmed via the supervisor-extraction recipe** — third sweep, run as 9 parallel extractions. Closes 6 of 9 body-length warnings; the remaining 3 are soft warnings (dense decision logic, accepted per the "≤12k tolerance" rule):
  ...
  - `refactoring-tobe/frontend-scaffolder` 15 433 → < 10 000 — 4 ref docs: `workspace-config.md`, `code-skeletons.md`, `streamlit-translations.md`, `app-shell.md`.
  - `refactoring-tobe/refactoring-tobe-supervisor` 14 419 → 11 251 (−22%, soft warning) — 3 new ref docs at `docs/refactoring-tobe/`: `inputs-and-flags.md`, `manifest-schema.md`, `sub-agents-roster.md` (alongside the 5 existing iter-6 docs).
  - `refactoring-tobe/phase4-challenger` 14 072 → 9 856 (−30%) — 2 ref docs: `checklist-templates.md`, `output-report-template.md`.
  - `refactoring-tobe/logic-translator` 13 907 → 10 493 (−25%, soft warning) — ref docs under `docs/refactoring-tobe/logic-translator/`.
  - `developers/developer-frontend` 13 711 → < 10 000 (−30%) — 2 ref docs under `docs/developers/developer-frontend/`: `per-framework-conventions.md`, `output-templates.md`.
  - `tobe-testing/tobe-testing-supervisor` 13 136 → 11 222 (−15%, soft warning) — extended `docs/tobe-testing/output-layout.md` with `## Manifest schema`, new `docs/tobe-testing/sub-agents.md`.
  - `documentation/wiki-writer` 13 035 → 9 748 (−31%) — 2 ref docs under `docs/documentation/wiki-writer/`: `page-templates.md`, `output-rules.md`.

  Same pattern as iter-2: each agent body keeps only `## Role`, `## When to invoke`, `## Reference docs` (anchor table), `## Inputs (from supervisor)`, `## Method` (decision logic), `## Outputs` (compressed), `## Stop conditions`, `## Constraints`. All output templates / schemas / boilerplate moved verbatim to ref docs. No content invented.
...
### Added
- **Progressive disclosure section in `how-to-write-a-capability.md` § "Skills — when and how to use them"**, documenting the Anthropic three-level loading model (frontmatter / SKILL body / `references/`) and the optional directory layout `claude-catalog/skills/<topic>/<skill-name>/{<skill-name>.md, references/*.md}`. Soft cap on body length: 3 000 words. Hard cap: 5 000 words (Anthropic limit). Closes the gap identified by the Anthropic auto-improving-agents review: "the registry's larger skills load all content every time, with no pattern for splitting reference material".
- **`iter_capability_files()` helper in `.github/scripts/validate_catalog.py`** that excludes `references/` and `scripts/` subdirectories from capability scanning. All 9 `rglob("*.md")` call sites in the validator now use this helper. Required for the directory layout above so that `references/*.md` and `scripts/*.md` (introduced in the upcoming optional-`scripts/` PR) are not validated as capabilities. Behaviour for the existing flat layout (`<topic>/<name>.md`) is unchanged.
