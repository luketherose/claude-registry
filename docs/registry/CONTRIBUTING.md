...
```bash
./scripts/new-capability.sh capability-name
# or for a skill:
./scripts/new-capability.sh --type skill skill-name
```
...
# Write plugins/<plugin>/evals/capability-name/evals.json + triggers.json        (at least 3 scenarios, written before the capability)
...
#   - frontmatter: only name and description (no model, no tools, no color)
...
# Write evals/skill-name-eval.md        (optional but recommended)
# Reference the skill from each consuming agent: `Skill` tool in the "## Skills" section,
#   or the "skills:" frontmatter field when the agent needs it on every run
# Add entry to CHANGELOG.md under "Unreleased"
...
- [ ] Skill file is in `skills/`, not `agents/`
