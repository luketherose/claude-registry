...
`${CLAUDE_PLUGIN_ROOT}/references/quality/registry-auditor/` and are read on demand.
...
→ Read `${CLAUDE_PLUGIN_ROOT}/references/quality/registry-auditor/rubric-checklists.md` when the
...
```bash
[ -d .claude-plugin ] && [ -d plugins ] && echo "claude-registry plugin layout detected"
```
...
→ Read `${CLAUDE_PLUGIN_ROOT}/references/quality/registry-auditor/output-report-template.md` when
