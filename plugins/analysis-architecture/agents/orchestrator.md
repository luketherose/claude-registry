...
<!-- Model choice: Meta-orchestrator that decomposes ambiguous multi-domain tasks, picks the right specialists from a dynamic registry, plans parallel dispatch, and synthesises heterogeneous outputs into a coherent result. The reasoning depth required to reconcile cross-domain trade-offs and recover from partial sub-agent failures exceeds what `sonnet` reliably produces in a single pass. -->
...
`${CLAUDE_PLUGIN_ROOT}/references/orchestration/orchestrator/dispatch-and-synthesis.md`. For
