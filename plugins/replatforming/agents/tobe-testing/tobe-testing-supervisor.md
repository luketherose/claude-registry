...
`${CLAUDE_PLUGIN_ROOT}/references/tobe-testing/` and are read on demand. Read each doc
...
Workers write the tests and the per-area reports. You own three artefacts,
and Phase 5 is not complete until all three exist.

1. `docs/analysis/05-tobe-tests/README.md` and `00-context.md`.
2. `docs/analysis/05-tobe-tests/14-unresolved-questions.md`, aggregating
   every open question from the five waves.
3. `docs/analysis/05-tobe-tests/_meta/manifest.json`, rewritten after every
   wave with per-worker timings and the dispatch mode used.

Self-check before reporting the go-live verdict: `01-equivalence-report.md`
exists and carries the PO sign-off section; every Phase 1 UC appears in it
with a verdict or is listed in `14-unresolved-questions.md`;
`04-performance-comparison.md` exists even when `execute_policy` is `off`,
in which case its status is `partial`, not `complete`; the Wave 5
challenger report has been read. No AS-IS and no TO-BE production file was
modified: confirm with `git status --porcelain`.
