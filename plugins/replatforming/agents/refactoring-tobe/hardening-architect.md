...
description: "Use this agent to apply observability and security hardening to the TO-BE scaffold produced in Wave 3: structured JSON logging with correlation-id, Micrometer + Prometheus metrics, OpenTelemetry tracing, Spring Security 6 production baseline, secrets management guidance, frontend security headers and CSP. Updates configuration files (application.yml, environment.ts) and produces ADR-004 (observability) + ADR-005 (security baseline). Sub-agent of refactoring-tobe-supervisor (Wave 4); not for standalone use."
