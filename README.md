...
| `refactoring-supervisor` | beta | **Refactoring workflow supervisor (opus, v1.2.0)**: top-level workflow for end-to-end refactoring/migration. Delegates phases sequentially to dedicated supervisors (Phase 0 indexing, Phase 1 functional analysis, Phase 2 technical analysis, Phase 3 baseline testing, Phase 4 TO-BE refactoring). Strict human-in-the-loop with schematic preview before each phase and execution-timing recap after. v1.2.0: when Phase 1 or 2 analysis is complete but the PDF/PPTX export is missing, offers a `regenerate-exports` choice that runs only the export wave without re-doing the analysis. v1.1.0: bootstrap detects existing phase outputs and asks the user explicitly per phase to skip / re-run / revise (no more silent auto-skip). |
